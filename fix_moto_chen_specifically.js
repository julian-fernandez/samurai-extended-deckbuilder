...
// Create a backup
fs.writeFileSync(
  path.join(__dirname, "public", "cards_v2_before_moto_chen_specific_fix.json"),
  JSON.stringify(cards, null, 2)
);
console.log("Created backup before Moto Chen specific fixes");
...
// Function to find XML card by name (handles experienced versions properly)
function findXmlCardByName(name) {
  ...
  // If it's an experienced card, try to find the specific experienced version
  if (name.includes(" - Experienced")) {
    // Try different experienced formats
    const expFormats = [
      name.replace(/\s*-\s*Experienced\s*$/, " - exp"),
      name.replace(/\s*-\s*Experienced\s*2\s*$/, " - exp2"),
      name.replace(/\s*-\s*Experienced\s*3\s*$/, " - exp3"),
      name.replace(/\s*-\s*Experienced\s*4\s*$/, " - exp4"),
      name.replace(/\s*-\s*Experienced\s*5\s*$/, " - exp5"),
    ];

    for (const expFormat of expFormats) {
      card = xmlCardList.find(
        (card) =>
          card.name && card.name.toLowerCase() === expFormat.toLowerCase()
      );
      if (card) return card;
    }
  }
...
// Main processing - focus on Moto Chen cards
console.log("\n=== FIXING MOTO CHEN CARDS SPECIFICALLY ===");

let cardsFixed = 0;
let cardsSkipped = 0;
const fixes = [];

// Find all Moto Chen cards
const motoChenCards = cards.filter(
  (card) => card.title && card.title[0] && card.title[0].includes("Moto Chen")
);

console.log(`Found ${motoChenCards.length} Moto Chen cards`);

for (const card of motoChenCards) {
...
console.log(`\n=== SUMMARY ===`);
console.log(`Moto Chen cards processed: ${motoChenCards.length}`);
